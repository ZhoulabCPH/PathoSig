...
class SwinTransformerBlock(nn.Module):
	...
	Args:
		...
		attn_drop (float, optional): 注意力机制 dropout rate. Default: 0.0
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C Wh Ww#1x96x64x64
        if self.norm is not None:
            Wh, Ww = x.size(2), x.size(3)  # 1x4x2x5
...
class SwinTransformerV1(nn.Module):
    ...
    def forward(self, x):  # 1x3x256x256
        x = self.patch_embed(x)  # 1x96x64x64
...
if 1:
    numhead=2
    dim=32*numhead
    wmsa = WindowAttention(dim=dim, window_size=(8, 8), num_heads=numhead)
    xx = torch.rand(15, 64, dim)
    # wmsa(xx)
    a=wmsa(xx)
    a=1
